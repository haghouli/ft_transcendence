...
export default class chatComponent {
    ...
    //         document.getElementById('scroll').innerHTML += await newComponent.render(item.id, item.name, item.image, status);
    //         newComponent.init();
    ...
    setElements = async () => {
        ...
        const promises = room_data.map(async (item) => {
            let status = item.is_online ? "online" : "offline";
    
            const newComponent = new TestComponent(item.id, item.name, item.image, status, testVar);
            
            // Await the CSS update before rendering
            await updateCss(newComponent.getCssPath());
    
            // Render the component and append to the scroll element
        ...
        // Append all rendered components to the scroll element
        document.getElementById('scroll').innerHTML = renderedComponents.join('');
    
        // Initialize each component after rendering
        room_data.forEach((item, index) => {
            const newComponent = new TestComponent(item.id, item.name, item.image, 'online');
            newComponent.init();
        });
    }
